...
export default function PagamentosPage() {
  ...
  const [filters, setFilters] = useState({ status: "", categoria: "", from: "", to: "" });
  ...
  async function marcarPago(id: string) {
    ...
    const { error } = await supabase.from("conta_pagar").update({ data_pagamento: dataPagamento }).eq("id", id);
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-4">
        ...
        <div>
          <button className="px-4 py-2 bg-sky-700 text-white rounded" onClick={() => setOpenModal(true)}>Novo pagamento</button>
        </div>
      </div>
      ...
      <table className="min-w-full bg-white rounded shadow">
        ...
        <tbody>
          {loading ? <tr><td colSpan={9} className="p-4">Carregando...</td></tr> :
            items.map(item => {
              const overdue = item.status === "vencido";
              return (
                <tr key={item.id} className={overdue ? "bg-red-50" : ""}>
                  <td className="p-3">{item.descricao}</td>
                  <td className="p-3">{item.categoria_id ?? "-"}</td>
                  <td className="p-3">{item.fornecedor_id ?? "-"}</td>
                  <td className="p-3">{item.data_vencimento}</td>
                  <td className="p-3">{item.data_pagamento ?? "-"}</td>
                  <td className="p-3"></td>
                  <td className="p-3">R$ {Number(item.valor).toFixed(2)}</td>
                  <td className="p-3"><span className={`px-2 py-1 rounded ${item.status === "vencido" ? "bg-rose-200 text-rose-800" : item.status === "pago" ? "bg-green-100 text-green-800" : "bg-gray-100 text-gray-800"}`}>{item.status}</span></td>
                  <td className="p-3">
                    {item.status !== "pago" && <button className="px-3 py-1 bg-green-600 text-white rounded" onClick={() => marcarPago(item.id)}>Marcar como pago</button>}
                  </td>
                </tr>
              );
            })
      ...
      <PaymentModal open={openModal} onClose={() => { setOpenModal(false); fetchItems(); }} />
    </div>
  );
}
